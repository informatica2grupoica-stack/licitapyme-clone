...
const [[doc]]: any = await pool.query(
  `SELECT documento_url_local FROM documentos_cache WHERE licitacion_codigo=? AND documento_nombre='ANEXO_2.docx'`,
  [codigo],
);
...
for (const c of analisis.completadosAuto) console.log(` "${c.etiqueta}" → ${c.campo} = "${c.valor}" (${c.via}) [formulario=${c.formulario ?? '(sin formulario)'}]`);
console.log('\npendientesCelda:', analisis.pendientesCelda.length);
for (const p of analisis.pendientesCelda) console.log(` "${p.etiqueta}" [${p.categoria}] formulario=${p.formulario ?? '(sin formulario)'}`);
console.log('\npendientesInline:', analisis.pendientesInline.length);
for (const p of analisis.pendientesInline) console.log(` "${p.contexto}" [${p.categoria}] formulario=${p.formulario ?? '(sin formulario)'}`);
console.log('\ntablas:', analisis.tablas.length);
for (const t of analisis.tablas) console.log(` formulario=${t.formulario ?? '(sin formulario)'} filas=${t.filas.length}`);
