    ...
    <div className="space-y-3">
      ...
      {escenarios.length > 0 && (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
          {escenarios.map(e => {
            const esElTipoElegido = elegidoTipo === e.tipo;
            const elegidoVigente = esElTipoElegido && elegidoCostoGuardado != null && Math.abs(e.costoTotal - elegidoCostoGuardado) <= 1;
            const elegidoDesactualizado = esElTipoElegido && !elegidoVigente;
            return (
            <div key={e.tipo} className={`bg-white rounded-xl border p-3 ${elegidoVigente ? 'border-emerald-300 ring-1 ring-emerald-100' : elegidoDesactualizado ? 'border-amber-300 ring-1 ring-amber-100' : e.esPrincipal ? 'border-teal-300 ring-1 ring-teal-100' : 'border-zinc-200'}`}>
              <div className="flex items-center justify-between gap-2">
                ...
                {elegidoDesactualizado && (
                  <span className="flex-shrink-0 flex items-center gap-1 text-[9.5px] font-bold text-amber-700 bg-amber-50 border border-amber-200 px-1.5 py-0.5 rounded-full">
                    <AlertTriangle size={10} /> Cambió — vuelve a elegir
                  </span>
                )}
              </div>
              {elegidoDesactualizado && (
                <p className="text-[10px] text-amber-600 mt-0.5">Se eligió por {fmtCLP(elegidoCostoGuardado!)} — este tipo de escenario ahora cuesta distinto.</p>
              )}
              ...
              {/* Desglose mercadería + flete (pedido explícito del usuario, 14-sep-2026: "estamos
                  trabajando con dinero y no podemos fallar" — el total de esta tarjeta SIEMPRE
                  incluye el flete interno estimado (spec §8.10.2), pero la suma de los productos de
                  abajo es SOLO mercadería. Sin este desglose, el total no calzaba con "precio × 7"
                  a simple vista y parecía un error — es la misma separación que ya se usa en la
                  Compuerta 1 de Aprobaciones). */}
              {(() => {
                const mercaderia = e.detalle.porProducto.reduce((s, p) => s + (p.subtotal || 0), 0);
                const flete = e.costoTotal - mercaderia;
                if (flete <= 0 && !e.fleteSinConfirmar) return null;
                // Si algún ítem elegido trae un flete real (leído del documento o tipeado a mano),
                // el rótulo lo dice explícito. Ya NO se adivina un monto interno (pedido explícito,
                // 15-sep-2026: "no me puedes poner 40 por defecto, eso lo tenemos que poner
                // nosotros ya que es dinero") — si nadie escribió cuánto cuesta el viaje, el total
                // de arriba queda con $0 de flete y esta línea avisa que ese $0 no está confirmado.
                const hayFleteReal = e.detalle.porProducto.some(p => p.fleteMonto != null);
                return (
                  <>
                    {flete > 0 && <p className="text-[10px] text-zinc-400">({fmtCLP(mercaderia)} mercadería + {fmtCLP(flete)} flete {hayFleteReal ? 'cobrado por el proveedor' : ''})</p>}
                    {e.fleteSinConfirmar && (
                      <p className="text-[10px] font-semibold text-amber-600 bg-amber-50 border border-amber-200 rounded px-1.5 py-0.5 mt-0.5 inline-block">
                        ⚠ Flete de retiro sin confirmar — agrega el monto real en la cotización antes de aprobar.
                      </p>
                    )}
                  </>
                );
              ...
              <div className="mt-2 pt-2 border-t border-zinc-100 space-y-1.5">
                {e.detalle.porProducto.map(p => {
                  // Detalle completo por ítem (pedido explícito, 15-sep-2026: "necesito que este
                  // todo detallado de cada uno" — la línea combinada de arriba suma mercadería+flete
                  // de TODO el escenario junto; acá se abre mercadería y flete de CADA producto por
                  // separado, con su propio total, para poder verificar la plata sin tener que
                  // restar a mano).
                  const fleteItem = p.fleteMonto ?? null;
                  const totalItem = p.subtotal != null ? p.subtotal + (fleteItem ?? 0) : null;
                  return (
                    <div key={p.productoId} className="text-[10.5px] bg-zinc-50/60 rounded-lg px-2 py-1.5">
                      <p className="text-zinc-700 font-medium truncate">{p.descripcion}</p>
                      {p.proveedor ? (
                        <>
                          <p className="text-zinc-500">
                            <span className="font-semibold text-zinc-700">{p.proveedor}</span>
                            {p.cantidad != null && ` · ${p.cantidad} un.`}
                            {p.plazoEntregaDias != null && ` · ${p.plazoEntregaDias} día(s)`}
                          </p>
                          <div className="mt-0.5 grid grid-cols-2 gap-x-2 text-zinc-500">
                            <span>Mercadería: <b className="text-zinc-700">{fmtCLP(p.subtotal)}</b></span>
                            <span>
                              Flete: <b className="text-zinc-700">{fleteItem != null ? fmtCLP(fleteItem) : (p.incluyeFlete ? 'incluido' : '$0 sin confirmar')}</b>
                            </span>
                          </div>
                          {totalItem != null && (
                            <p className="mt-0.5 text-zinc-600">Total ítem: <b className="text-zinc-800">{fmtCLP(totalItem)}</b></p>
                          )}
                        </>
                      ) : (
              ...
              {puedeOperar && !elegidoVigente && confirmandoTipo === e.tipo && (
                <div className="mt-2 space-y-1.5">
                  <input value={justificacionEscenario} onChange={ev => setJustificacionEscenario(ev.target.value)}
                    placeholder="¿Por qué eliges este escenario en vez de Más rápido? (spec §8.10.4)"
                    className="w-full text-[11px] border border-amber-300 rounded-lg px-2 py-1.5 outline-none focus:ring-1 focus:ring-amber-500" />
                  <div className="flex items-center gap-1.5">
                    <button onClick={() => elegir(e.tipo, justificacionEscenario.trim())} disabled={!justificacionEscenario.trim() || eligiendo === e.tipo}
                      className="text-[11px] font-semibold text-white bg-amber-600 hover:bg-amber-700 disabled:opacity-50 px-2.5 py-1.5 rounded-lg">
                      {eligiendo === e.tipo ? <Loader2 size={12} className="animate-spin" /> : 'Confirmar'}
                    </button>
                    <button onClick={() => { setConfirmandoTipo(null); setJustificacionEscenario(''); }} className="text-[11px] text-zinc-400 hover:text-zinc-600">Cancelar</button>
                  </div>
                </div>
              )}
              {puedeOperar && !elegidoVigente && confirmandoTipo !== e.tipo && (
                <button
                  onClick={() => { if (e.tipo === 'MAS_RAPIDO') elegir(e.tipo, null); else { setConfirmandoTipo(e.tipo); setJustificacionEscenario(''); } }}
                  disabled={eligiendo === e.tipo}
                  className={`mt-2 flex items-center gap-1 text-[11px] font-semibold text-white disabled:opacity-50 px-2.5 py-1.5 rounded-lg ${elegidoDesactualizado ? 'bg-amber-600 hover:bg-amber-700' : 'bg-zinc-800 hover:bg-zinc-900'}`}>
                  {eligiendo === e.tipo ? <Loader2 size={12} className="animate-spin" /> : <CheckCircle2 size={12} />} {elegidoDesactualizado ? 'Confirmar el nuevo monto' : 'Elegir este escenario'}
                </button>
              )}
            </div>
            );
