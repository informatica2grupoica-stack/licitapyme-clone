...
REGLA CLAVE — DOS IDENTIDADES DISTINTAS, NUNCA LAS MEZCLES: esta empresa postula como PERSONA JURÍDICA, así que en cada anexo conviven dos identidades que se escriben distinto:

  (A) LA EMPRESA — es EL OFERENTE. Sinónimos que SIEMPRE apuntan a la empresa: "oferente", "proponente", "participante", "razón social", "nombre o razón social", "contribuyente", "postulante", "el/la empresa", "prestador", "proveedor". Se llena con perfil_empresa (razon_social, rut, direccion, giro…). El RUT que corresponde acá es el RUT DE LA EMPRESA (76.xxx.xxx-x), NUNCA la cédula del representante.

  (B) LA PERSONA NATURAL que firma por la empresa — es el REPRESENTANTE LEGAL. Sinónimos que SIEMPRE apuntan a esa persona: "representante legal", "apoderado", "firmante", "encargado de la propuesta", "contacto para la licitación", "administrador de contrato", "declarante". Se llena con perfil_representante_legal (representante_nombre, representante_rut, representante_cargo). El RUT/cédula acá es la CÉDULA DE LA PERSONA, NUNCA el RUT de la empresa.

Las de (B) sí son todas la MISMA persona de la ficha entre ellas (el encargado de la propuesta, el contacto y el administrador de contrato son el representante legal): si un bloque pide "Nombre completo", "Cargo", "Cédula de identidad", "Teléfono" o "Correo" bajo cualquiera de esos títulos, complétalo con esa persona — no lo dejes pendiente por dudar de CUÁL persona es.

Pero (A) y (B) NUNCA son lo mismo: "Nombre del Oferente" NO se llena con el nombre del representante legal, y "Nombre del Representante Legal" NO se llena con la razón social. Es el error que más se comete en estos formularios.

CÓMO DECIDIR cuando la etiqueta es ambigua: manda la palabra que CALIFICA a la casilla, no la que esté más cerca. "Nombre del representante legal del proponente" pide la PERSONA (el núcleo es "representante legal"; "del proponente" solo dice a quién representa). "Razón social del proponente" pide la EMPRESA. Si la etiqueta dice solo "Nombre:" y el bloque que la encabeza es de identificación del oferente, pide la EMPRESA; si ese bloque es el pie de firma o un bloque de "representante legal", pide la PERSONA.

CASO FRECUENTE — las dos juntas en el pie de firma: "Firma del Representante Legal / Nombre del Proponente" son DOS casillas distintas: la primera es la persona, la segunda es la razón social. Léelas por separado, nunca repitas el mismo valor en ambas.
